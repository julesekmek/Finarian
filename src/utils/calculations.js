...
export function calculatePortfolioTotals(assets) {
  ...
  const totalCurrent = assets.reduce((sum, asset) => {
    const quantity = parseFloat(asset.quantity) || 0;
    const currentPrice =
      parseFloat(asset.current_price) || parseFloat(asset.purchase_price) || 0;
...
export function calculateAssetMetrics(asset) {
  ...
  const gainPercent =
    totalPurchaseValue > 0 ? (unrealizedGain / totalPurchaseValue) * 100 : 0;
...
export function calculateCategoryMetrics(assets) {
  ...
  Object.values(categoryData).forEach((cat) => {
    cat.gainPercent =
      cat.totalInvested > 0 ? (cat.totalGain / cat.totalInvested) * 100 : 0;
