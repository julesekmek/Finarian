...
export default function PortfolioChart({ userId }) {
  ...
  const yAxisDomain = useMemo(() => {
    if (!filteredHistory || filteredHistory.length === 0) return ['auto', 'auto']
    ...
    const effectivePadding = range < maxValue * 0.01 ? maxValue * 0.02 : padding
      ...
      return (
        <div className="bg-dark-card border border-border-subtle px-4 py-3 rounded-xl shadow-card">
          ...
          {Math.abs(changePercent) > 0.01 && (
            <p className={`text-xs font-medium ${changePercent >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>
              {changePercent >= 0 ? '+' : ''}{changePercent.toFixed(2)}% depuis le début
            </p>
          )}
        </div>
      )
  ...
  const getTrendColor = () => {
    ...
    if (kpiMetrics.trend === 'positive') return { stroke: '#F1C086', fill: 'url(#colorPrimary)' }
    if (kpiMetrics.trend === 'negative') return { stroke: '#EF4444', fill: 'url(#colorRed)' }
      ...
      {kpiMetrics && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          ...
          >
            ...
            <div className="flex items-center gap-1">
              ...
              <p className={`text-xl font-bold ${
                (kpiMetrics.currentValue - totalInvested) >= 0 ? 'text-accent-green' : 'text-accent-red'
              }`}>
                {(kpiMetrics.currentValue - totalInvested) >= 0 ? '+' : ''}
          ...
          >
            ...
            <div className="flex items-center gap-1">
              {((kpiMetrics.currentValue - totalInvested) / totalInvested * 100) >= 0 ? (
                <TrendingUp className="w-4 h-4 text-accent-green" />
              ) : (
                <TrendingDown className="w-4 h-4 text-accent-red" />
              )}
              <p className={`text-xl font-bold ${
                ((kpiMetrics.currentValue - totalInvested) / totalInvested * 100) >= 0 ? 'text-accent-green' : 'text-accent-red'
              }`}>
                {((kpiMetrics.currentValue - totalInvested) / totalInvested * 100) >= 0 ? '+' : ''}
                {totalInvested > 0 ? ((kpiMetrics.currentValue - totalInvested) / totalInvested * 100).toFixed(2) : '0.00'}%
              </p>
            </div>
          </motion.div>
        </div>
      ...
      >
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: stroke }}></div>
            <span className="text-sm font-medium text-text-secondary">
              {period === 'all' ? 'Toutes les données' : `${period} derniers jours`}
            </span>
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip 
                ...
                cursor={{ stroke: stroke, strokeWidth: 1, strokeDasharray: '5 5' }}
              />
              <Area 
                type="monotone" 
                dataKey="value" 
                ...
                activeDot={{ 
                  ...
                  strokeWidth: 3, 
                  stroke: '#1A1F2E',
                  style: { filter: 'drop-shadow(0 0 4px rgba(241, 192, 134, 0.6))' }
                }}
